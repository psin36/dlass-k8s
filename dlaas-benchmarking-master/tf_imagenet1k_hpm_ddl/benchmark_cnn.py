...
_DEFAULT_PARAMS = {
    ...
    #   ddl: Distributed training using DDL library. Runs workers using
    #       an MPI framework (e.g. Open/Spectrum MPI). Each worker runs training on
    #       single GPU, and averages gradients using NCCL or MPI all-reduce.
    'variable_update':
        _ParamSpec('string', 'parameter_server',
                   'The method for managing variables: parameter_server, '
                   'replicated, distributed_replicated, independent, '
                   'distributed_all_reduce, ddl'),
    'all_reduce_spec':
    ...
    'cross_replica_sync':
        _ParamSpec('boolean', True, ''),
    'ddl_device': #GJ17 not sure about this!!! FIXME
        _ParamSpec('string', '',
                   'Device to do DDL all-reduce on: empty (default), '
                   'cpu or gpu. Default will utilize GPU.'),
    'ddl_options':
        _ParamSpec('string', '-mode r',
                   'Options passed to ddl.init(): -mode r (default).'),
...
def create_config_proto(params):
  ...
  if params.variable_update == 'ddl':
    import ddl
    config.gpu_options.visible_device_list = str(ddl.local_rank())
  return config
...
class BenchmarkCNN(object):
  ...
  def __init__(self, params):
    ...
    if self.params.variable_update == 'ddl' and self.params.num_gpus > 1:
      raise ValueError('DDL benchmarks require num_gpus=1 on each worker')

    if self.params.variable_update == 'ddl' and self.params.job_name:
      raise ValueError('job_name should not be specified for DDL.')
    ...
    elif self.params.variable_update in ('independent', 'ddl'):
      if self.job_name:
        raise ValueError('Invalid variable_update in distributed mode: %s' %
                         self.params.variable_update)
    ...
    if self.params.variable_update == 'ddl':
      import ddl
      # Pass along any ddl specific options:
      ddl.init(self.params.ddl_options)
  ...
  def print_info(self):
    ...
    elif self.params.variable_update == 'ddl':
      import ddl
      device_list = ['ddl:%d' % idx for idx in range(ddl.size())]
      num_workers = ddl.size()
      log_fn('DDL #hosts:  %d' % ddl.num_hosts())
      log_fn('DDL rank:    %d' % ddl.rank())
      log_fn('DDL size:    %d' % ddl.size())
      log_fn('DDL gpuid:   %d' % ddl.local_rank())
    else:
    ...
    if self.params.variable_update == 'ddl' and self.params.ddl_device:
      log_fn('DDL on:      %s' % self.params.ddl_device)
    log_fn('==========')
  ...
  def _benchmark_cnn(self):
    ...
    sv = tf.train.Supervisor(
        # For the purpose of Supervisor, all DDL workers are 'chiefs',
        # since we want session to be initialized symmetrically on all the
        # workers.
        is_chief=is_chief or self.params.variable_update == 'ddl',
        # Log dir should be unset on non-chief workers to prevent DDL
  ...
  def add_forward_pass_and_gradients(
    ...
    with tf.device(self.devices[rel_device_num]):
      ...
      if self.params.variable_update == 'ddl':
        import ddl
        if self.params.ddl_device:
          ddl_device = '/%s:0' % self.params.ddl_device
        else:
          ddl_device = ''
        # All-reduce gradients using DDL.
        grads = ddl.allreduce_n(grads, device_dense=ddl_device)
