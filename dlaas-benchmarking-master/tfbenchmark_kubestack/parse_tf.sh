...
while [ $i -lt $len ]; do
  ...
  if [[ $string == *$testseq* ]]; then
    ...
    #echo $st
    j="$st $string"
    thpt=$(echo $j | awk '{split($0,a,":"); split(a[2],d," "); split(d[2],b,"_"); split(b[7],c,".log"); gsub (" ", "",d[1]); print b[1]","int(b[2])","int(b[3])","int(b[4])",disk,"b[5]","b[6]","c[1]","d[1]}')
    #echo "cmd to execute" 
    #echo $cmd
    #eval $cmd
    ...
    #gpu=$(cat $string1 | awk 'BEGIN{count=0; sum=0} {if(NR>100 && NR <1000) {if($13 ~ "%") {sum = sum+ $13; count=count+1;}}} END{print sum/count}')
    gpu=$(cat $string1 | awk 'BEGIN{count=0; sum=0} {if($13 ~ "%") {sum = sum+ $13; count=count+1;}} END{print sum/count}')
    #echo $gpu
    #line=$string,$thpt,$gpu
    line=$thpt,$gpu
    echo $line
  fi
  ((i++))
